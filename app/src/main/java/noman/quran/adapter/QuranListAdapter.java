...
public class QuranListAdapter extends BaseAdapter {
    ...
    public QuranListAdapter(Context context, ArrayList<IndexListModel> dataList) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        QuranListAdapter.ViewHolder holder = null;
        LayoutInflater mInflater = (LayoutInflater) mContext.getSystemService(Activity.LAYOUT_INFLATER_SERVICE);

        if (convertView == null) {
            convertView = mInflater.inflate(R.layout.row_surah_list, null);

            holder = new QuranListAdapter.ViewHolder();
            ...
            holder = (QuranListAdapter.ViewHolder) convertView.getTag();
        }

        holder.tvSurahNo.setText("" + (listSurahs.get(position).getItemPosition() + 1));
        holder.tvMakkiMadni.setText(listSurahs.get(position).getPlaceOfRevelation().trim() +", Juz: "+listSurahs.get(position).getParaIndex());

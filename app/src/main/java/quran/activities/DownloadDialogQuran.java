...
public class DownloadDialogQuran extends Activity {
	...
	public boolean checkDownloadStatus() {
		...
		DBManagerQuran dbObj = new DBManagerQuran(DownloadDialogQuran.this);
		dbObj.open();
			...
			do
			{
				...
				if(name.equals(audioName) || name.equals(Constants.QuranFile2))
				{
					try
					{
						...
						{
							chkDownload = FileUtils.chkDownloadStatus(DownloadDialogQuran.this, cursor, tempName, refId, surahPos);
						}
						else
						{
							chkDownloadStatus = FileUtils.checkOneAudioFile(DownloadDialogQuran.this, tempName, surahPos);
						}
	...
	public void onDownloadSurah() {
		...
		if(isNetworkConnected())
		{
			...
			{
				Toast toast = Toast.makeText(DownloadDialogQuran.this,"Not Enough Memory" + "(" + String.format("%.2f", sizeRequired * 1E-6) + "MB Wajib)", Toast.LENGTH_SHORT);
			...
			else
			{
				...
				{
					Intent serviceIntent = new Intent(DownloadDialogQuran.this, ServiceClass.class);
		...
		{
			Toast toast = Toast.makeText(DownloadDialogQuran.this,R.string.toast_network_error, Toast.LENGTH_SHORT);
			toast.setGravity(Gravity.CENTER, 0, 0);
			toast.show();
				}
